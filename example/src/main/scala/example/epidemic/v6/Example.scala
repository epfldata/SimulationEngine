...
object MainInit {
    val liftedMain = meta.classLifting.liteLift {
        def apply(population: Int, p: Double, sbm: Boolean, blocks: Int, interval: Int): List[Actor] = {
            val citizens = (1 to population).map(c => { new Person(Random.nextInt(90) + 10, interval) })
