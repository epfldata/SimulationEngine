...
class SimulateUntil {
    OptimizationConfig.logControllerEnabled = true
    var timeseries: Iterable[Iterable[Serializable]] = null

    def driver(totalTurn: Long, port: Int = 25251): Unit = {
    ...
    def machine(mid: Int, actors: IndexedSeq[Actor], totalTurn: Long, port: Int = 0): Unit = {
        val config = ConfigFactory.parseString(s"""
            akka.remote.artery.canonical.port=$port
    ...
    def apply(actors: IndexedSeq[Actor], totalTurn: Long, 
            cond: Iterable[Iterable[Serializable]] => Boolean, role: String= "Standalone", port: Int = 25251): Unit = {
        val config = ConfigFactory.parseString(s"""
            akka.remote.artery.canonical.port=$port
        ...
        val actorSystem = ActorSystem(AkkaExp(totalTurn, totalWorkers, actors, cond), "SimsCluster", config)
        Await.ready(actorSystem.whenTerminated, 10.days)
        println("Simulation ends!")
    }
}
