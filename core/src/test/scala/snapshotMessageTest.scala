...
class SnapshotMessageTest extends FlatSpec {
    ...
    "The snapshot" should "capture messages not processed in the base messaging layer" in {
    ...
    "The snapshot" should "capture messages not processed in the Akka messaging layer" in {
        val agents = generated.meta.test.snapshot.InitData()
        val c = new SimulationConfig(agents, 1)
